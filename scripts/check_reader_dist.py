...
for reader in ("none", "vertical"):
    torch.manual_seed(0)
    cfg = GPTConfig(sequence_len=256, vocab_size=32768, n_layer=10, n_head=5, n_kv_head=5,
                    n_embd=640, window_pattern="L", reader=reader)
    with torch.device("meta"):
        model = GPT(cfg)
    model.to_empty(device=device)
    model.init_weights()
    opt = model.setup_optimizer()                       # asserts reader params wired
    ...
    print0(f"reader={reader:>8}: 3 distributed optimizer steps OK | final loss={loss.item():.3f}")
