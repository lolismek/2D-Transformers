...
Self-validates against three ground-truth anchors from the repo before printing anything:
  ...
  * d10 WideReader L2 reader FLOPs   = 648.8e6 -> total 2.122x  (README / nanochat_10p2_reader.md)
    ...
    python3 scripts/reader_flops.py
"""
import math
...
# ====================================================================================================
# 3) READER CELLS  H in {14,18,22} x V in {2,4,8,10}  -> matched equal-compute baseline depth h'
# ====================================================================================================
HS = [14, 18, 22]
VS = [2, 4, 8, 10]
print("\n" + "=" * 78)
...
print("=" * 78)
print(f"{'H':>3} {'V':>3} {'n_embd':>6} {'fpt x base':>10} {'tokens':>9} {'total FLOPs':>11} "
      f"{'C/Cbase(H)':>10} {'match hp':>8}")
cells = []
for h in HS:
    for V in VS:
        r = reader_point(h, V)
        cbase_h = base_point(h)["flops"]
        hp = matched_depth(r["flops"])
        cells.append((h, V, r, hp))
        print(f"{h:>3} {V:>3} {r['e']:>6} {r['mult']:>9.2f}x {fmt(r['tokens']):>9} {fmt(r['flops']):>11} "
              f"{r['flops']/cbase_h:>9.1f}x {hp:>7.1f}")
...
print(f"all matched h' strictly greater than their reader H?  "
      f"{all(nearest_even(matched_depth(reader_point(h,V)['flops'])) > h for h in HS for V in VS)}")
